...
class Network(nn.Module):
  ...
  def forward(self, input):
    ...
    for i, cell in enumerate(self.cells):
      if cell.reduction:
        ...
        n = 3
        start = 2
        weights2 = self.betas_reduce[0:2]
        for i in range(self._steps-1):
          end = start + n
          tw2 = self.betas_reduce[start:end]
          start = end
          n += 1
        ...
        n = 3
        start = 2
        weights2 = self.betas_normal[0:2]
        for i in range(self._steps-1):
          end = start + n
          tw2 = self.betas_normal[start:end]
          start = end
          n += 1
          weights2 = torch.cat([weights2,tw2],dim=0)
  ...
  def genotype(self):
    ...
    n = 3
    start = 2
    weightsr2 = self.betas_reduce[0:2]
    weightsn2 = self.betas_normal[0:2]
    for i in range(self._steps-1):
      end = start + n
      #print(self.betas_reduce[start:end])
      tw2 = self.betas_reduce[start:end]
      tn2 = self.betas_normal[start:end]
      start = end
      n += 1
      weightsr2 = torch.cat([weightsr2,tw2],dim=0)
      weightsn2 = torch.cat([weightsn2,tn2],dim=0)
    gene_normal = _parse(self.alphas_normal.data.cpu().numpy(),weightsn2.data.cpu().numpy())
    gene_reduce = _parse(self.alphas_reduce.data.cpu().numpy(),weightsr2.data.cpu().numpy())
